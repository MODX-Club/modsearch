...
class modSearch{
    ...
    */
    ...
    protected function parseFields(modResource $doc, array & $words){
        # $this->parseWords($doc->pagetitle, $words);
        # $this->parseWords($doc->longtitle, $words);
        # $this->parseWords($doc->content, $words);
        
        if($fields = $this->modx->getOption('modsearch.index_fields', null, 'pagetitle,longtitle,content')){
            $fields = explode(",", $fields);
            foreach($fields as $field){
                if(
                    $field = trim($field)
                    AND $content = $doc->get($field)
                ){
                    $this->parseWords($content, $words);
                }
            }
        }
    }
